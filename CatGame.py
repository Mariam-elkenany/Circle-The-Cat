...
class Game(object):
    def __init__(self, size):  #initial state
        self.cat_i = size//2
        self.cat_j = size//2
        self.size  = size
    ...
    def init_random_blocks(self,cat): #random chosen blocks
    ...
    def target(self,i,j,dir): #goal state
        out=[i,j] 
        if   dir == "E":
            out=[i,j+1]
        elif dir =="W":
    ...
    def apply_move(self,move,maximizing_player):
            ...
            if maximizing_player:
                ...
                self.tiles[move[0]][move[1]] = 6        
                self.tiles[self.cat_i][self.cat_j] = 0  
                self.cat_i = move[0]
                self.cat_j = move[1]

                    
    def max_Value(self, upper_game, move, maximizing_player, depth, maxdepth): #max
        if self.time_left()<5:
    ...
    def min_Value(self, upper_game, move, maximizing_player, depth, maxdepth): #min
        if self.time_left()<5:
            self.terminated=True
            return 0
        game=copy.deepcopy(upper_game)
    ...
    def minimax(self, max_depth=float("inf"), maximizing_player=True): #minimax algorithm
